#!/usr/bin/env node
/**
 * Generate 10-minute drive-time isochrones for Málaga (103 municipios).
 *
 * Mirrors the Euskadi script `scripts/generate-isochrones.js` —
 * same rate limit, retry logic, and progress-resume behaviour —
 * pointed at Málaga's boundaries and output paths.
 *
 * Usage:
 *   ORS_API_KEY=your_key node data/es/malaga/generate_isochrones.js
 *
 * OpenRouteService free tier: 40 req/min, 2000/day.
 * Malaga needs 103 muni calls + 1 province merge (no API) = ~3 min.
 *
 * Outputs:
 *   data/es/malaga/isochrones/municipios/{ine_code}.geojson
 *   data/es/malaga/isochrones/provincias/29.geojson
 *   data/es/malaga/isochrones/progress.json
 */
...
const MUNI_ISO_DIR = path.join(DIR, 'isochrones', 'municipios');
const PROV_ISO_DIR = path.join(DIR, 'isochrones', 'provincias');
const PROGRESS_PATH = path.join(DIR, 'isochrones', 'progress.json');
...
const RANGE_SECONDS = 600; // 10 minutes
...
async function main() {
  ...
  // Merge provincia 29 (single-province merge, but keep same pattern as Euskadi)
  console.log('\nMerging Málaga provincia isochrone...');
  try {
    ...
    if (merged) {
      ...
      const outPath = path.join(PROV_ISO_DIR, '29.geojson');
      fs.writeFileSync(outPath, JSON.stringify(provGeoJSON));
      console.log(`  ✓ Málaga (${count} municipios merged)`);
    }
  } catch (e) {
    console.error('Error merging provincia isochrone:', e.message);
  }

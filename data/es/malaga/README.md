...
| **Status (2026-04-27)** | **Research delivered.** `malaga_nla_research_filled.xlsx` committed alongside the template. `merge_nla_research.py` ingested the xlsx and updated `malaga_facilities.json`: `nla_m2` field renamed to `nla_sqm` (matches Euskadi schema). Per-facility metadata added: `nla_sqm`, `constructed_area_sqm`, `nla_confidence`, `nla_source_url`, `nla_notes`, `nla_estimated`. **Coverage**: 92 / 99 facilities with NLA (12 direct from operator websites, 80 estimated via Catastro × 0.70). **7 facilities** (all Independent) without NLA — the researcher couldn't find a source. **Province total**: 59,391 m² NLA (70,149 m² constructed area). |

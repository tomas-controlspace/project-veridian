...
- **Isochrone files**: 13 Basque municipios have no isochrone data (ORS API errors or no road access from the centroid). All 103 Málaga municipios have both 10-min and 20-min isochrones. The app and the `prepare-data.js` catchment computation handle missing files gracefully — those munis just have null `catch_*` fields.

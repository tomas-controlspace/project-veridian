...
data/es/                            # Source data (NOT served to browser)
  ...
  facilities/basque_facilities.json  # 61 storage facilities (46 legacy Bizkaia/Álava + 15 canonical Donostia/Gipuzkoa)
...
public/data/               # Build output (served to browser via fetch). UNION of all regions.
  ...
  metrics_regions.json     # One record per region (PV / AN). Top-level region aggregate keyed by region code.
  ...
  facilities.json          # Combined facility list (160 sites: 61 Euskadi + 99 Málaga) with facility_type + size_tier
...
- Density per km²: 20%
- Housing market turnover rate (annual transactions ÷ total dwellings; falls back to provincial rate `housing_turnover_annual_prov ÷ Σ provincial total_dwellings` when muni-level data is missing): 10%
...
**Canonical facility schema** (Málaga 2026-04-23 + Donostia 2026-04-30 follow this; the 46 Bizkaia/Álava legacy entries are kept on a different shape — see Gotchas):
...
- **Isochrone files**: All 252 Basque + 103 Málaga municipios have both 10-min and 20-min isochrones. The original Euskadi run silently dropped 13 munis on rate-limit retries because the `while (retries < 3)` loop exited without marking the muni as 'error' (so progress.json was clean and a re-run was needed to recover them). Patched in all four generator scripts (`scripts/generate-isochrones{,-20}.js` and `data/es/malaga/generate_isochrones{,-20}.js`) — exhausted retries now log + mark + count the failure. Re-runs are still safe and idempotent because the skip check is `progress[code] === 'done'`. The app + `prepare-data.js` catchment computation still tolerate missing files (catchments default to null).
